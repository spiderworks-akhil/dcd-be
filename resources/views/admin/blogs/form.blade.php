                                                    ...
                                                    <div>
                                                        <div class="row m-0">
                                                            ...
                                                            <div class="card">
                                                                <div class="card-header">
                                                                    Content
                                                                    <a href="javascript:void(0)" id="add-more-content" class="float-right text-primary"><i class="fas fa-plus mr-2"></i> Add More</a>
                                                                </div>
                                                                @php
                                                                    $content_count = ($obj->id && !empty(json_decode($obj->content)))?count(json_decode($obj->content, true)):0;
                                                                @endphp
                                                                    ...
                                                                    @if($content_count == 0)
                                                                        <div class="card">
                                                                            <div class="card-body">
                                                                                <div class="form-group col-md-12">
                                                                                    <label>Title</label>
                                                                                    <input type="text" name="content[{{$content_count}}][title]" class="form-control text-direction" >
                                                                                </div>
                                                                                <div class="form-group col-md-12">
                                                                                    <label>Description</label>
                                                                                    <textarea name="content[{{$content_count}}][description]" id="content[{{$content_count}}][description]" class="form-control text-direction editor" rows="2"></textarea>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    @else
                                                                        @foreach(json_decode($obj->content) as $key=>$content)

                                                                            <div class="card content-item">
                                                                                <div class="card-body">
                                                                                    <div class="form-group col-md-12">
                                                                                        <label>Title</label>
                                                                                        <input type="text" name="content[{{$key}}][title]" class="form-control text-direction" value="{{$content->title}}" >
                                                                                    </div>

                                                                                    <div class="form-group col-md-12">
                                                                                        <label>Description</label>
                                                                                        <textarea name="content[{{$key}}][description]" id="content_{{$key}}_description" class="form-control text-direction editor" rows="2" >{{$content->description}}</textarea>
                                                                                    </div>
                                                                                    @if($key > 0)
                                                                                        <div class="form-group col-md-12">
                                                                                            <a href="javascript:void(0)" class="float-right text-danger remove-content">Remove</a>
                                                                                        </div>
                                                                                    @endif
...
var content_count  = "{{$content_count}}";
        ...
        $(document).on('click', '#add-more-content', function(){
            content_count++;
            let html = '<div class="card content-item"><div class="card-body"><div class="form-group col-md-12"><label>Title</label><input type="text" name="content['+content_count+'][title]" class="form-control text-direction"></div><div class="form-group col-md-12"><label>Description</label><textarea name="content['+content_count+'][description]" id="content_'+content_count+'_description" class="form-control editor text-direction" rows="2"></textarea></div><div class="form-group col-md-12"><a href="javascript:void(0);" class="float-right text-danger remove-content">Remove</a></div></div></div>';
            $('#blog-content-holder').append(html);
            setTimeout( function() {
                initEditor(document.querySelector( '#content_'+content_count+'_description' ));
            }, 500);
        })
