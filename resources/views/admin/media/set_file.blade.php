...
@if($file)
	...
	@else
		<div class="thumbnail text-center" >
			<div class="card">
			@if($file->media_type == 'Image')
		  		<a href="{{route('admin.media.show', [encrypt($file->id)])}}" class="webadmin-open-ajax-popup item-meida create-image-icon" title="View Image Details" data-popup-size="large"><img src="{{ asset($file->file_path) }}?ver={{time()}}" width="200px"></a>
		  	@else
		  		<a href="{{asset($file->file_path)}}" class="item-meida" target="_blank"><img src="{{ asset($file->thumb_file_path) }}"></a>
		  	@endif
		  <div class="card-body">
...
@endif
	<div class="media-container" @if($file) style="display: none;" @endif id="add-new-{{$media_id}}">
		<p class="media-content text-center" ><a href="{{route('admin.media.popup', ['popup_type'=>$popup_type, 'type'=>$type, 'holder_attr'=>$holder_attr, 'title'=>$title, 'related_id'=>0, 'media_id'=>$media_id, 'display'=>$display])}}" class="webadmin-open-ajax-popup create-image-icon" title="Media Images" data-popup-size="xlarge" >{{--<img src="{{asset('admin/assets/images/add_image.png')}}" style="width:150px;">--}} Set {{$title}}</a></p>
	</div>
</div>
